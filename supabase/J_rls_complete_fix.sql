-- ════════════════════════════════════════════════════════════
--  الشارع المصري — RLS FIX الشامل (النسخة النهائية)
--  آمن 100% — يعدّي اللي اتعمل ومش بيوقف على errors
-- ════════════════════════════════════════════════════════════
...
CREATE POLICY "settings_insert" ON public.site_settings FOR INSERT WITH CHECK (public.is_admin());
CREATE POLICY "settings_update" ON public.site_settings FOR UPDATE USING (public.is_admin()) WITH CHECK (public.is_admin());
CREATE POLICY "settings_delete" ON public.site_settings FOR DELETE USING (public.is_admin());
...
CREATE POLICY "arts_read"   ON public.articles FOR SELECT USING (status = 'published' OR public.is_admin());
CREATE POLICY "arts_insert" ON public.articles FOR INSERT WITH CHECK (public.is_admin());
CREATE POLICY "arts_update" ON public.articles FOR UPDATE USING (public.is_admin()) WITH CHECK (public.is_admin());
CREATE POLICY "arts_delete" ON public.articles FOR DELETE USING (public.is_admin());
...
CREATE POLICY "prof_update"    ON public.profiles FOR UPDATE USING (auth.uid() = user_id OR public.is_admin()) WITH CHECK (auth.uid() = user_id OR public.is_admin());
CREATE POLICY "prof_admin_all" ON public.profiles FOR ALL USING (public.is_admin());
...
-- ═══ 9. subscribers — محمي بـ DO block (ممكن يكون view) ═══
DO $$
BEGIN
  IF EXISTS (
    ...
    EXECUTE 'DROP POLICY IF EXISTS "sub_insert" ON public.subscribers';
    EXECUTE 'DROP POLICY IF EXISTS "sub_read"   ON public.subscribers';
    EXECUTE 'DROP POLICY IF EXISTS "sub_write"  ON public.subscribers';
    EXECUTE 'DROP POLICY IF EXISTS "sub_delete" ON public.subscribers';
    EXECUTE $p$CREATE POLICY "sub_insert" ON public.subscribers FOR INSERT WITH CHECK (true)$p$;
    EXECUTE $p$CREATE POLICY "sub_read"   ON public.subscribers FOR SELECT USING (auth.uid() IS NOT NULL)$p$;
    EXECUTE $p$CREATE POLICY "sub_delete" ON public.subscribers FOR DELETE USING (public.is_admin())$p$;
  END IF;
END $$;
...
DROP POLICY IF EXISTS "com_delete" ON public.comments;
CREATE POLICY "com_read"   ON public.comments FOR SELECT USING (is_approved = true OR public.is_admin());
CREATE POLICY "com_insert" ON public.comments FOR INSERT WITH CHECK (true);
CREATE POLICY "com_update" ON public.comments FOR UPDATE USING (public.is_admin()) WITH CHECK (public.is_admin());
CREATE POLICY "com_delete" ON public.comments FOR DELETE USING (public.is_admin());

-- ═══ 11. advertisements — محمي بـ DO block ═══
DO $$ BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.tables
    ...
    EXECUTE 'DROP POLICY IF EXISTS "ads_update" ON public.advertisements';
    EXECUTE 'DROP POLICY IF EXISTS "ads_delete" ON public.advertisements';
...
CREATE POLICY "assets_admin_upload" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'site-assets' AND public.is_admin());
CREATE POLICY "assets_admin_update" ON storage.objects FOR UPDATE USING (bucket_id = 'site-assets' AND public.is_admin());
CREATE POLICY "assets_admin_delete" ON storage.objects FOR DELETE USING (bucket_id = 'site-assets' AND public.is_admin());

-- ═══════════════════════════════════════════════
-- 13. STORAGE — media bucket
-- ═══════════════════════════════════════════════
INSERT INTO storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
VALUES (
...
CREATE POLICY "media_admin_upload" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'media' AND public.is_admin());
