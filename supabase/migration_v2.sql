-- EG STREET NEWS: Complete DB Migration
-- Run in: Supabase Dashboard > SQL Editor

-- 1. articles: add missing SEO columns
ALTER TABLE articles
  ADD COLUMN IF NOT EXISTS meta_title       TEXT,
  ADD COLUMN IF NOT EXISTS meta_description TEXT;

-- 2. breaking_news: add slug
...
DROP POLICY IF EXISTS comments_manage ON comments;
...
CREATE POLICY comments_manage ON comments FOR ALL USING (auth.uid() IS NOT NULL);
...
DROP POLICY IF EXISTS art_tags_public ON article_tags;
DROP POLICY IF EXISTS art_tags_write ON article_tags;
CREATE POLICY art_tags_public ON article_tags FOR SELECT USING (true);
...
CREATE POLICY arts_public ON articles FOR SELECT USING (status = 'published' OR auth.uid() IS NOT NULL);
