-- ============================================================
-- EgStreet Complete Migration — Run this in Supabase SQL Editor
-- ============================================================
...
CREATE POLICY "Admin manage breaking" ON public.breaking_news FOR ALL USING (
  EXISTS (SELECT 1 FROM public.user_roles WHERE user_id=auth.uid() AND role IN ('super_admin','editor_in_chief','journalist'))
);
...
CREATE TABLE IF NOT EXISTS public.daily_views (
  ...
  city TEXT, view_count INTEGER DEFAULT 1,
...
CREATE POLICY "Admin read daily_views" ON public.daily_views FOR SELECT USING (
  EXISTS (SELECT 1 FROM public.user_roles WHERE user_id=auth.uid() AND role IN ('super_admin','editor_in_chief'))
);
...
CREATE POLICY "Admin read visitor_logs" ON public.visitor_logs FOR SELECT USING (
  EXISTS (SELECT 1 FROM public.user_roles WHERE user_id=auth.uid() AND role IN ('super_admin','editor_in_chief'))
);
...
INSERT INTO public.site_settings (key, value) VALUES
  ...
  ('feature_breaking_ticker','true'),
  ('feature_ads','true'),
  ('feature_saved_articles','true'),
  ('feature_social_share','true'),
  ('feature_copy_protection','false'),
  ('feature_news_scraper','true'),
  ('footer_message','جريدة الشارع المصري — صحافة تضرم عقلك'),
  ('logo_url',''),
  ('primary_color','#c41e2a'),
  ('site_description_ar','أخبار مصر والعالم العربي لحظة بلحظة'),
  ('site_description_en','Egypt and Arab world news')
