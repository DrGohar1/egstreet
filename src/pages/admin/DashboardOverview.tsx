...
import {
  ...
  PenTool, AlertTriangle, Calendar
} from "lucide-react";
import {
...
const DashboardOverview = () => {
  ...
  const statCards = [
    { label: t("إجمالي المقالات", "Total Articles"), value: stats.articles, icon: FileText, color: "text-primary", bg: "bg-primary/10" },
    { label: t("منشورة", "Published"), value: stats.published, icon: CheckCircle, color: "text-green-500", bg: "bg-green-500/10" },
    { label: t("مراجعة معلقة", "Pending Review"), value: stats.pending, icon: Clock, color: "text-yellow-500", bg: "bg-yellow-500/10", alert: stats.pending > 0 },
    { label: t("مسودات", "Drafts"), value: stats.drafts, icon: PenTool, color: "text-muted-foreground", bg: "bg-muted" },
    { label: t("أخبار عاجلة", "Breaking News"), value: stats.breaking, icon: Zap, color: "text-destructive", bg: "bg-destructive/10" },
    { label: t("المستخدمون", "Users"), value: stats.users, icon: Users, color: "text-blue-500", bg: "bg-blue-500/10" },
    { label: t("المشاهدات", "Views"), value: stats.totalViews.toLocaleString(), icon: Eye, color: "text-purple-500", bg: "bg-purple-500/10" },
    { label: t("المشتركون", "Subscribers"), value: stats.subscribers, icon: Mail, color: "text-emerald-500", bg: "bg-emerald-500/10" },
    { label: t("التعليقات", "Comments"), value: stats.comments, icon: MessageSquare, color: "text-orange-500", bg: "bg-orange-500/10" },
    { label: t("الأقسام", "Categories"), value: stats.categories, icon: Layers, color: "text-indigo-500", bg: "bg-indigo-500/10" },
  ];
  ...
  return (
    <div className="space-y-6">
        ...
        <Card className="border-yellow-500/50 bg-yellow-500/5">
          <CardContent className="flex items-center gap-3 py-3">
            ...
            <p className="text-sm text-foreground">
              {stats.pending > 0 && (
                <span>
                  {t(`${stats.pending} مقالة بانتظار المراجعة`, `${stats.pending} article(s) pending review`)}
                </span>
              )}
              {stats.pending > 0 && stats.pendingComments > 0 && " · "}
              {stats.pendingComments > 0 && (
                <span>
                  {t(`${stats.pendingComments} تعليق بانتظار الموافقة`, `${stats.pendingComments} comment(s) pending approval`)}
                </span>
              )}
            </p>
          </CardContent>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
        {statCards.map((card) => (
          <Card key={card.label} className="news-card-hover relative overflow-hidden">
        ...
        <CardContent>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              ...
              <Button key={action.to} asChild variant="outline" className="h-auto py-4 flex-col gap-2">
                <Link to={action.to}>
