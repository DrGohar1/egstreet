  ...
  useEffect(() => {
    (async () => {
      const [catR, artR, acR] = await Promise.all([
        ...
        supabase.from("articles").select("id,title,slug,excerpt,featured_image,published_at,custom_author_name,views,category_id,is_featured,is_breaking").eq("status", "published").order("published_at", { ascending: false }).limit(30),
